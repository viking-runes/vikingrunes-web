...
export async function generate_stake_psbt(stakerAddress, stakerPubkey, stakePool: IResponseStakeItem, networkFee: number) {
  const tx = new Psbt({
    network: network,
  });
...
function locked_p2tr(pubkey, script) {
...
export async function claim(stakerAddress, stakePool: IResponseStakeItem, stakerPubkey, networkFee, txid) {
