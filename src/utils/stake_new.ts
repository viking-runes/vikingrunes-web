...
export async function claim(claimItem: IGraphQLClaimItem, stakerAddress: string, stakerPubkey: string, feeRate: number = 30) {
  const unlock_time = claimItem.locked_time;

  // const fee = order.network_fee;
  const fee = config.claimVSize * feeRate;

  const selected_utxo = await select_staker_utxo(stakerAddress, +claimItem.amount + fee, 0);

  const locked_script = lock_script(unlock_time, stakerPubkey);

  const commit_p2tr = locked_p2tr(stakerPubkey, locked_script);

  const amount = +claimItem.amount;
  ...
  claim_tx.addInput({
    hash: selected_utxo.txid,
    index: selected_utxo.vout,
    witnessUtxo: {
      script: Buffer.from(selected_utxo.scriptPk, 'hex'),
      value: selected_utxo.satoshis,
    },
    publicKey: stakerPubkey,
    sighashType: Transaction.SIGHASH_ALL,
    tapInternalKey: Buffer.from(stakerPubkey, 'hex'),
  ...
  claim_tx.addOutput({
    address: stakerAddress,
    value: +selected_utxo.satoshis + amount - fee - 546,
    // value: amount - fee,
  });
