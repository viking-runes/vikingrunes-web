...
export async function txStake(body) {
  const { data } = await axios.post(`${config.stakeUrl}/stake/api/tx/stake`, body);
  if (data.code !== 200) throw Error(data.message);
  return data.data;
}

export async function fetchOrders(body) {
  const { data } = await axios.post(`${config.stakeUrl}/stake/api/v1/orders`, body);
  if (data.code !== 200) throw Error(data.message);
  return data.data;
}

export async function fetchOrder(uuid) {
  const { data } = await axios.post(`${config.stakeUrl}/stake/api/v1/order/${uuid}`, {});
  if (data.code !== 200) throw Error(data.message);
  return data.data;
}
