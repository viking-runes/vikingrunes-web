...
import { FeeDialog } from '@/pages/staking/components';
...
export default function ClaimTable() {
  ...
  const handleClaimConfirm = async () => {
    feeDialog.handleClose();

    // TODO choose fee rate modal
    try {
      const stakePsbt = await claim(selectedClaimItem, wallet.address, getSignedPublicKey(), feeRate.getCurrentSelectedRate());
      ...
      enqueueSnackbar('Claim tx sent', { variant: 'success' });
    } catch (error) {
      console.log(error);
      enqueueSnackbar(error?.message, {
        variant: 'error',
  ...
  return (
    <TableContainer>
      ...
      <FeeDialog
        networkVSize={config.claimVSize}
        open={feeDialog.open}
        handleClose={feeDialog.handleClose}
        onConfirm={() => {
          handleClaimConfirm();
        }}
      />
